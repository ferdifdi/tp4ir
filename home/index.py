...
class InvertedIndex:
    ...
    def __init__(self, index_name, postings_encoding, directory=''):
        ...
        self.index_file_path = os.path.join(directory, os.path.dirname(__file__) +"\\index\\"+index_name+'.index')
        self.metadata_file_path = os.path.join(directory, os.path.dirname(__file__) +"\\index\\"+index_name+'.dict')
...
class InvertedIndexReader(InvertedIndex):
    ...
    def get_postings_list(self, term):
        ...
        start_position, number_of_postings, length_in_bytes, tf_length = self.postings_dict[term]
        self.index_file.seek(start_position, 0)
        postings_list = self.postings_encoding.decode(self.index_file.read(length_in_bytes))
        tf_list = self.postings_encoding.decode(self.index_file.read(tf_length))
...
class InvertedIndexWriter(InvertedIndex):
    ...
    def append(self, term, postings_list, tf_list):
        ...
        encoded_pl = self.postings_encoding.encode(postings_list)
        encoded_tfl = self.postings_encoding.encode_tf(tf_list)
        if self.terms != []:
            (start_position, number_of_postings, len_pl, len_tfl) = self.postings_dict[self.terms[-1]]
            self.postings_dict[term] = (start_position+len_pl+len_tfl,len(postings_list),len(encoded_pl),len(encoded_tfl))
        else:
            self.postings_dict[term] = (0,len(postings_list),len(encoded_pl),len(encoded_tfl))

        self.terms.append(term)
        self.index_file.write(encoded_pl)
        self.index_file.write(encoded_tfl)

        for i in range(len(postings_list)):
            doc_pl = postings_list[i]
            val_tfl = tf_list[i]
            if (doc_pl in self.doc_length):
                cur_val = self.doc_length[doc_pl]
                new_val = cur_val + val_tfl
                self.doc_length[doc_pl] = new_val
            else:
                self.doc_length[doc_pl] = val_tfl
